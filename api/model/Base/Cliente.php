...
abstract class Cliente implements ActiveRecordInterface
{
    ...
    public function setNomeCliente($v)
    {
        ...
        if ($this->nome_cliente !== $v) {
            $this->nome_cliente = $v;
            $this->modifiedColumns[ClienteTableMap::COL_NOME_CLIENTE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ClienteTableMap::translateFieldName('NomeCliente', TableMap::TYPE_PHPNAME, $indexType)];
            $this->nome_cliente = (null !== $col) ? (string) $col : null;
            $this->resetModified();
            ...
            return $startcol + 2; // 2 = ClienteTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ClienteTableMap::COL_NOME_CLIENTE)) {
            $modifiedColumns[':p' . $index++]  = 'nome_cliente';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'idcliente':
                        ...
                        break;
                    case 'nome_cliente':
                        $stmt->bindValue($identifier, $this->nome_cliente, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setNomeCliente($arr[$keys[1]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ClienteTableMap::COL_NOME_CLIENTE)) {
            $criteria->add(ClienteTableMap::COL_NOME_CLIENTE, $this->nome_cliente);
        }
    ...
    {
        $copyObj->setNomeCliente($this->getNomeCliente());
        if ($makeNew) {
            $copyObj->setNew(true);
